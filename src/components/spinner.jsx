...
const Spinner = (props) => {
            ...
            <div className="bg-[#fff7f7] flex flex-col justify-center items-start pl-5 w-[80%] rounded-md">
                 <div className="pt-5 ">
                    ...
                    <div className="flex gap-1 bg-white p-2 mt-2">
                        <input type="text" name="" id=""  value={`${percent}%`} className="font-bold text-black" onChange={
                            (e)=>{
                                setPercentChance(e.target.value)
                            }
                        }/>
                        <div className="flex gap-2 ">
                            <button className="bg-black text-white font-bold p-2 text-center text-sm" onClick={()=>{ calculateMinDeal()}}>Min</button>
                            <button className="bg-black text-white font-bold p-2 text-center text-sm" onClick={()=>{ calculateDeal(10)}}>10%</button>
                            <button className="bg-black text-white font-bold p-2 text-center text-sm" onClick={()=>{ calculateDeal(25)}}>25%</button>
                            <button className="bg-black text-white font-bold p-2 text-center text-sm" onClick={()=>{ calculateDeal(50)}}>50%</button>
                            <button className="bg-black text-white font-bold p-2 text-center text-sm" onClick={()=>{ calculateMaxDeal()}}>Max</button>
                        </div>
                    </div>
                 </div>
                 ...
                 <div className="flex gap-1 bg-white p-3 mt-2">
                    <input type="text" className="w-[100%] font-bold text-black" name="" id="" value={`$ ${deal}`} onChange={
                        (e)=>{
  ...
  <input type="range" min="0" max="100" value="50" step="1" className="w-full h-3 rounded-full bg-gray-300 appearance-none focus:outline-none focus:bg-blue-500 focus:ring-2 focus:ring-blue-600" />
  </div>
                </div>
            </div>
        </div>
